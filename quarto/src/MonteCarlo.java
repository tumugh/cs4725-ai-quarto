...
public class MonteCarlo {
	...
	public static int[][] getPossibleMoves(QuartoBoard board, Integer piece) {
		ArrayList<int[]> movesList = new ArrayList<int[]>();
		Set<QuartoBoard> boardSet = new HashSet<QuartoBoard>();
		...
		for (int row = 0; row < board.getNumberOfRows(); row++) {
			for (int col = 0; col < board.getNumberOfColumns(); col++) {
				if (!board.isSpaceTaken(row, col)) {
					...
					//if the board generated by making that move exists in the boardset, then don't add the move to the move list
					boolean contains = false;
					for (QuartoBoard boardSetBoard : boardSet) {
						boardSetBoard.printBoardState();
						copyBoard.printBoardState();
					...
					if (!contains) {
						int[] moves = {row, col};
						movesList.add(moves);
						Set<QuartoBoard> symmetricBoards = findSymmetricBoards(copyBoard);
	...
	public static Set<QuartoBoard> findSymmetricBoards(QuartoBoard board) {
		Set<QuartoBoard> set = new HashSet<QuartoBoard>();
	...
	public static QuartoBoard rotateBoard(QuartoBoard board) {
		QuartoBoard copyBoard = new QuartoBoard(board);
		transpose(copyBoard.board);
		reverseRows(copyBoard.board);
		return copyBoard;
	}
	
	public static void transpose(QuartoPiece[][] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = i; j < array[0].length; j++) {
				QuartoPiece x = array[i][j];
				array[i][j] = array[j][i];
				array[j][i] = x;
			}
		}
	...
	public static void reverseRows(QuartoPiece[][] array) {
		for(int i = 0; i < array.length; i++){
		    for(int j = 0; j < array[i].length / 2; j++) {
		        QuartoPiece temp = array[i][j];
...
//		 }
//		 
		 int[][] moves = getPossibleMoves(board, 1);
